...
const renderCategories = (categories) => {
  ...
  for (const category of categories) {
    $("#nwcategory_render").innerHTML += `<tr class="border-b ">
...
const buttonDeleteCategory = (categoryId) => {
  ...
  $(".modal_delete_button").addEventListener("click", (e) => {
    const categoriesId = $(".modal_delete_button").getAttribute(
      "data-id-modal"
    );
...
const modalDeleteCategory = (categoryId) => {
  const currentDataModalOperations = getData("operations").filter(
  ...
  const currentDataModalCategories = getData("categories").filter(
...
const tabChangeToEditionOfCategory = (categoryId) => {
  ...
  const categorySelect = getData("categories").find(
...
const renderOperations = (operations) => {
  ...
  if (operations.length) {
    ...
    for (const operation of operations) {
      $("#nwoperation_render").innerHTML += `<tr class="border-b">
                        ...
                        <td class="p-2">
                            <button class="text-violeta mr-2" onclick="tabChangeToEditionOfOperation('${
                              operation.id
                            }')">Editar</button>
                            <button class="text-red-500" onclick="modal_delete.showModal(),buttonOperationRemove('${
                              operation.id
                            }')">Eliminar</button>
                        </td>
                    </tr>`;
...
const buttonOperationRemove = (operationId) => {
  ...
  $(".modal_delete_button").addEventListener("click", () => {
    const operationsId = $(".modal_delete_button").getAttribute(
      "data-id-modal"
    );
...
const tabChangeToEditionOfOperation = (operationsId) => {
  hideTab([
    ...
    ".reports_tab",
    ".new_operation_title",
    ".add_nwoperation_button",
  ]);
  showTab([
    ".nwoperation_tab",
    ".edit_operation_title",
    ".edit_editoperation_button",
  ]);

  $("#edit_editoperation_button").setAttribute(
    "data-id-operations",
    operationsId
  );
  const operationSelect = getData("operations").find(
...
const byDate = (operations, fromDate) => {
  return operations.filter(
    (operation) => new Date(operation.date) >= new Date(fromDate)
  );
};
...
const updateDate = () => {
  const date = new Date();
  $("#date_nwoperation_input").value =
    date.getFullYear().toString() +
    "-" +
    (date.getMonth() + 1).toString().padStart(2, 0) +
    "-" +
    date.getDate().toString().padStart(2, 0);
  $("#from_select_input").value =
    date.getFullYear().toString() +
    "-" +
    (date.getMonth() + 1).toString().padStart(2, 0) +
    "-" +
...
const validateFormOperation = (field) => {
  ...
  switch (field) {
    case "nameOperation":
      if (nameOperation === "") {
        $("#description_nwoperation_input").classList.add(
          "border-red-500",
          "border",
          "border-2"
        );
    ...
    case "amountOperation":
      if (!amountOperation) {
        $("#amount_nwoperation_input").classList.add(
          "border-red-500",
          "border",
          "border-2"
        );
      } else {
        $("#amount_nwoperation_input").classList.remove(
          "border-red-500",
          "border",
          "border-2"
        );
...
const categoryHighestEarnings = () => {
  ...
  for (const operation of operations) {
    if (balancedCategory[operation.category]) {
      ...
      balancedCategory[operation.category] =
        operation.type === "earning" ? operation.amount : -operation.amount;
    }
  }
...
const categoryReportSummary = (category) => {
  ...
  const filterOperation = currentDataOperations.filter(
...
const initializeApp = () => {
  ...
  $("#categories_section_button").addEventListener(
    "click",
  ...
  $("#cancel_editcategory_button").addEventListener(
    "click",
    tabChangeToCategories
  );
  ...
  $("#add_nwcategory_button").addEventListener("click", (e) => {
    ...
    if (functionSaveCategory.name === "") {
      $("#name_nwcategory_input").classList.add(
        "border-red-500",
        "border",
        "border-2"
      );
  ...
  $("#edit_editcategory_button").addEventListener("click", (e) => {
    e.preventDefault();
    const categoriesId = $("#edit_editcategory_button").getAttribute(
      "data-id-categories"
    );
  ...
  $("#edit_editoperation_button").addEventListener("click", (e) => {
    ...
    const operationsId = $("#edit_editoperation_button").getAttribute(
      "data-id-operations"
    );
  ...
  $("#category_select_filter").addEventListener("input", (e) => {
    ...
    if (filterSelected === "Todas") {
      ...
      const filterOperation = currentData.filter(
        (operation) => operation.category === filterSelected
      );
  ...
  $("#type_select_filter").addEventListener("input", (e) => {
    ...
    if (filterSelected === "todos") {
      ...
      const currentData = getData("operations");
      const filterOperation = currentData.filter(
        (operation) => operation.type === filterSelected
      );
      renderOperations(filterOperation);
    }
  });
  ...
  // VALITADION EVENT
  $("#description_nwoperation_input").addEventListener("blur", () =>
    validateFormOperation("nameOperation")
  );
  $("#amount_nwoperation_input").addEventListener("blur", () =>
    validateFormOperation("amountOperation")
  );
};
